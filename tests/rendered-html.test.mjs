...
test("renders the production flashcard home screen", async () => {
  ...
  assert.match(html, /https:\/\/youtu\.be\/VRqWc-waiDI/);
  ...
  assert.match(html, /data-testid="toggle-lesson-nejimaki"/);
  assert.doesNotMatch(html, /data-testid="start-all-tenten0718"/);
  assert.doesNotMatch(html, /data-testid="start-basic-order-quiz"/);
  assert.doesNotMatch(html, /クイズ問題一覧/);
  ...
  assert.doesNotMatch(html, /全(?:<!-- -->)?50(?:<!-- -->)?問/);
  assert.doesNotMatch(html, /解き直しカード/);
  assert.match(html, /管理画面/);
  assert.equal(html.indexOf("7/18　てんてん先生") < html.indexOf("7/16　ねじまき鳥先生"), true);
  assert.equal(html.indexOf("7/16　ねじまき鳥先生") < html.indexOf("7/14　てんてん先生　基礎講義復習"), true);
  assert.equal(html.indexOf("7/14　てんてん先生　基礎講義復習") < html.indexOf("7/2　ねじまき鳥先生　基礎講義②"), true);
...
test("publishes the expected social metadata", async () => {
  ...
  assert.match(html, /rel="icon" href="https:\/\/ensuku-basic-flashcards\.kobotenmitsu\.chatgpt\.site\/icons\/serina\.png"/);
  assert.match(html, /rel="apple-touch-icon" href="https:\/\/ensuku-basic-flashcards\.kobotenmitsu\.chatgpt\.site\/icons\/serina\.png"/);
});
